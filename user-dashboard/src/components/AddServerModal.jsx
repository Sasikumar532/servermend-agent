        ...
        created ? (
          <>
            <Link
              ...
              onClick={onClose}
              className="inline-flex items-center rounded-lg border border-border px-4 py-2 text-sm font-medium hover:bg-default"
            >
          ...
          <Button type="submit" form="add-server-form" disabled={submitting}>
    ...
    >
      {created ? (
        <div className="flex flex-col gap-4">
          ...
          <dl className="grid grid-cols-[auto_1fr] items-center gap-x-4 gap-y-2 text-sm">
            ...
            <dd className="flex items-center gap-2">
              <code className="min-w-0 rounded bg-background px-1.5 py-0.5 font-mono break-all">
                {created.serverId}
              </code>
        ...
        <form id="add-server-form" className="flex flex-col gap-4" onSubmit={handleSubmit}>
          ...
          <Field label="Hostname" hint="(optional, for display only)">
            <TextInput value={hostname} onChange={setHostname} name="hostname" placeholder="web-1.example.com" autoFocus />
          </Field>
        </form>
      )}
    </Modal>
